...
{
    private const int VerificationTokenExpiryMinutes = 5;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (user is null)
        {
            throw new ArgumentException("Liên kết xác minh không hợp lệ.");
        }

        if (user.EmailVerificationTokenExpiresAt is null
            || DateTime.UtcNow > user.EmailVerificationTokenExpiresAt)
        {
            throw new ArgumentException("Liên kết xác minh đã hết hạn. Vui lòng đăng nhập lại để nhận email mới.");
        }
