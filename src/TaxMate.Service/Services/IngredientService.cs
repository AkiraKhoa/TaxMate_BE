...
public class IngredientService : IIngredientService
{
    ...
    public async Task<IngredientResponse> CreateAsync(CreateIngredientRequest request)
    {
        ...
        var exists = await _unitOfWork.Ingredients
            .AnyAsync(x => x.Name.ToLower() == request.Name.ToLower() && !x.IsDeleted);

        if (exists)
            throw new Exception($"Ingredient with name '{request.Name}' already exists.");
        ...
        await _unitOfWork.Ingredients.AddAsync(entity);
    ...
    {
        var entity = await _unitOfWork.Ingredients.GetByIdAsync(id);
        if (entity is null)
            throw new Exception($"Ingredient with id '{id}' not found.");

        if (entity.IsDeleted)
            throw new Exception($"Ingredient with id '{id}' has been deactivated.");

        // Check duplicate name (exclude current)
        var duplicate = await _unitOfWork.Ingredients
        ...
        _unitOfWork.Ingredients.Update(entity);
        await _unitOfWork.SaveChangesAsync();
    ...
    public async Task DeactivateAsync(Guid id)
    {
        ...
        if (entity.IsDeleted)
            throw new Exception($"Ingredient with id '{id}' is already deactivated.");
    ...
    {
        var (items, totalCount) = await _unitOfWork.Ingredients
